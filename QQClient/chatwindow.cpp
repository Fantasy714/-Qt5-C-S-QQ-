...
#define InsertHeight 14 //每增加一行文字气泡需要的增加的高度

#define MinWindowMaxPicWidth 560 //最小窗口可容纳最宽图片宽度
...
QWidget *ChatWindow::CreateWidget(bool isMe, MsgType MsgType, QString Msg)
{
    ...
    case itsMsg:
    {
        ...
        if(isMe)
        {
            MsgLab->setStyleSheet("QLabel{background-color:rgb(18,183,245);color:white;padding-top:6px;padding-bottom:6px;"
        ...
        {
            MsgLab->setStyleSheet("QLabel{background-color:rgb(229,229,229);color:black;padding-top:6px;padding-bottom:6px;"
                                  "padding-left:8px;padding-right:8px;border-style:none;border-radius:8px;}");
...
QString ChatWindow::GetFileSize(const int &size)
{
    int integer = 0; //整数
    int decimal = 0; //小数
    QString unit = "B";
...
void ChatWindow::GetProgressInfo(int friAcc, QString fileName, int speed, int value, QTime RestTime,bool isMe)
{
