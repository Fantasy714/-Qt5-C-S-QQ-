...
public:
    ...
    void ReplyToJson(QString type,QString pwd,QString result = "", QString fileName = ""); //将信息转换为json
    void SendReply(QByteArray jsondata,QString fileName);
...
signals:
    ...
    void UserOnLine(int acc,QTcpSocket * sock); //用户上线则发送该tcp套接字加入服务器在线用户哈希表中
private:
    Qqsqldata sql; //连接数据库
